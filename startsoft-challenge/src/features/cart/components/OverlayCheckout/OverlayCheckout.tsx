...
function OverlayCheckoutContent({ onClose }: ContentProps) {
  ...
  // previne scroll do body quando o overlay estiver aberto
  useEffect(() => {
...
export function OverlayCheckout({ isOpen, onClose }: Props) {
  return (
    <AnimatePresence>
      {isOpen ? <OverlayCheckoutContent onClose={onClose} /> : null}
    </AnimatePresence>
  );
}
